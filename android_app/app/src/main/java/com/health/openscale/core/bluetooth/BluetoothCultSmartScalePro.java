...
public class BluetoothCultSmartScalePro extends BluetoothCommunication {
    ...
    private boolean sendUserProfile() {
        try {
            ...
            byte[] userProfile = getUserProfile(userId, selectedUser);
            ...
            writeBytes(CULT_SCALE_SERVICE, CONTROL_CHARACTERISTIC_FFF2, userProfile);
    ...
    private byte[] getUserProfile(int userId, ScaleUser selectedUser) {
        ...
        byte checksum = 0;
        for (byte profileDatum : profileData) {
            checksum ^= profileDatum;
        }
    ...
    private boolean startMeasurement() {
        try {
            ...
            if (!deviceConfigured) {
                ...
                sendMessage(R.string.info_is_not_available, 0);
            }
            ...
            writeBytes(CULT_SCALE_SERVICE, CONTROL_CHARACTERISTIC_FFF2, startCommand);
    ...
    public void onBluetoothNotify(UUID characteristic, byte[] value) {
        ...
        logDebugInfo("NOTIFY-" + characteristic.toString().substring(4, 8).toUpperCase(), value);

        Timber.d("Received notification from %s: [%s]", characteristic, byteInHex(value));
        ...
        if (data.length >= 3) {
            ...
            switch (statusType) {
                ...
                case (byte) 0xBE: // Error conditions
                    ...
                    sendMessage(R.string.trisa_scale_not_paired, 0);
                    break;
                default:
    ...
    private void parseWeightData(byte[] data) {
        try {
            ...
            if (data.length >= 6) {
                ...
                try {
                    int weightRaw = parser.getIntValue(FORMAT_UINT16, 3, ByteOrder.LITTLE_ENDIAN);
                ...
                if (!weightFound) {
                    try {
                        int weightRaw = parser.getIntValue(FORMAT_UINT16, 2, ByteOrder.LITTLE_ENDIAN);
                        weight = weightRaw / 100.0f;
                        if (weight >= 10.0f && weight <= 300.0f) {
                            weightFound = true;
                ...
                if (!weightFound) {
                    try {
                        int weightRaw = parser.getIntValue(FORMAT_UINT16, 1, ByteOrder.LITTLE_ENDIAN);
                        weight = weightRaw / 100.0f;
                        if (weight >= 10.0f && weight <= 300.0f) {
                            weightFound = true;
                ...
                if (!weightFound && data.length >= 8) {
                    try {
                        int weightRaw = parser.getIntValue(FORMAT_UINT32, 2, ByteOrder.LITTLE_ENDIAN);
            ...
            if (weightFound) {
                ...
                for (int i = 0; i < Math.min(data.length - 1, 5); i++) {
                    try {
                        int raw1 = parser.getIntValue(FORMAT_UINT16, i, ByteOrder.LITTLE_ENDIAN);
        ...
        try {
            ...
            if (data.length >= 20) {
                ...
                for (int pos : weightPositions) {
                    if (!hasValidWeight) {
                        try {
                            int weightRaw = parser.getIntValue(FORMAT_UINT16, pos, ByteOrder.LITTLE_ENDIAN);
                            weight = weightRaw / 100.0f;
                ...
                if (hasValidWeight) {
                    ...
                    try {
                        float fat = extractPercentageValueWithParser(parser, baseOffset);
                        float water = extractPercentageValueWithParser(parser, baseOffset + 2);
                        float muscle = extractPercentageValueWithParser(parser, baseOffset + 4);
                        float bone = extractPercentageValueWithParser(parser, baseOffset + 6) / 10.0f;
                        float visceral = extractPercentageValueWithParser(parser, baseOffset + 8) / 10.0f;
                    ...
                    if (validMetrics < 3) {
                        ...
                        try {
                            ...
                            float fat2 = parser.getIntValue(FORMAT_UINT8, 10, ByteOrder.LITTLE_ENDIAN) / 10.0f;
                            float water2 = parser.getIntValue(FORMAT_UINT8, 12, ByteOrder.LITTLE_ENDIAN) / 10.0f;
                            float muscle2 = parser.getIntValue(FORMAT_UINT8, 14, ByteOrder.LITTLE_ENDIAN) / 10.0f;
                            float bone2 = parser.getIntValue(FORMAT_UINT8, 16, ByteOrder.LITTLE_ENDIAN) / 100.0f;
                            float visceral2 = parser.getIntValue(FORMAT_UINT8, 17, ByteOrder.LITTLE_ENDIAN) / 10.0f;
     ...
     */
    private float extractPercentageValueWithParser(BluetoothBytesParser parser, int lowByte) {
        try {
            // Use BluetoothBytesParser for consistent 16-bit parsing
            // Pass position and ByteOrder as required by the method
            int value = parser.getIntValue(FORMAT_UINT16, lowByte, ByteOrder.LITTLE_ENDIAN);
            ...
            if (data.length >= 4) {
                ...
                for (int i = 0; i < Math.min(data.length - 1, 4); i++) {
                    ...
                    if (weight100 >= 10.0f && weight100 <= 300.0f) {
                        interpretation.append(String.format(Locale.US, "Weight@pos%d=%.1fkg; ", i, weight100));
                    } else if (weight10 >= 10.0f && weight10 <= 300.0f) {
                        interpretation.append(String.format(Locale.US, "Weight@pos%d=%.1fkg(/10); ", i, weight10));
                    }
                }
                ...
                for (int i = 0; i < Math.min(data.length - 1, 6); i++) {
                    ...
                    if (percent > 5.0f && percent <= 100.0f) {
                        interpretation.append(String.format(Locale.US, "Percent@pos%d=%.1f%%; ", i, percent));
                    }
                }
